...
%         save(f_name, 'myMatrix2', '-ascii');
        ...
        avg_rec=zeros(1,16);
        for m=1:16
            avg_rec(m)=mean(record(m,:));
        end
        stastic=zeros(1,4);
        allfes = Avg_FES;
        DIM = num2str(Par.n);
        DIMNAME = 'D';
        f_name = num2str(I_fno);
        FF='F';
        folder_path  = ['.\record\KIMODE\', num2str(Par.n), 'D\F', num2str(I_fno)];
        if ~exist(folder_path, 'dir')
            mkdir(folder_path);
        end
        
        stastic(1)=min(outcome);stastic(2)=max(outcome);stastic(3)=mean(outcome);stastic(4)=std(outcome);
        save(fullfile(folder_path, 'avg_rec.mat'), 'avg_rec');
        save(fullfile(folder_path, 'stastic.mat'), 'stastic');
        save(fullfile(folder_path, 'record.mat'), 'record');
        save(fullfile(folder_path, 'allfes.mat'), 'allfes');
    end
end
