...
% Modified to output cluster_prob for analysis.
% =========================================================================
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,res_det,F,cr, cluster_history, cluster_prob_out,labels_out] = ...
...
count_S = zeros(1,3);
if any(op_1), count_S(1) = max(0, mean(diff2(op_1==1))); end
if any(op_2), count_S(2) = max(0, mean(diff2(op_2==1))); end
if any(op_3), count_S(3) = max(0, mean(diff2(op_3==1))); end
if any(count_S)
...
for kc = 1:K_use
    ...
    c_counts = zeros(1,3);
    if any(op_1(idx)), c_counts(1) = max(0, mean(diff2(op_1(idx)==1))); end
    if any(op_2(idx)), c_counts(2) = max(0, mean(diff2(op_2(idx)==1))); end
    if any(op_3(idx)), c_counts(3) = max(0, mean(diff2(op_3(idx)==1))); end
    if any(c_counts)
...
cluster_prob_prev = cluster_prob;
centers_prev = centers;
prev_K = K_use;

%% [Output Modification]: Return the current cluster probabilities
cluster_prob_out = cluster_prob; 

%% update population
fitx(I==1)= fitx_new(I==1);
xold(I == 1, :) = x(I == 1, :);
x(I == 1, :) = ui(I == 1, :);
if exist('goodF','var') && ~isempty(goodF) && size(goodF,1)==1, goodF = goodF'; end
if exist('goodCR','var') && ~isempty(goodCR) && size(goodCR,1)==1, goodCR = goodCR'; end
num_success_params = 0;
if exist('goodCR','var') && ~isempty(goodCR), num_success_params = numel(goodCR); end
...
%% [Output Modification]
cluster_prob_out = cluster_prob; 
labels_out = labels; % <--- 新增：把当前的聚类标签传出去
end